<?php
    ...
    if(isset($_POST))
    {
        if(isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['promotion']) && !empty($_POST['promotion'])){
              
            $id = strip_tags($_GET['id']);
            $promotion = strip_tags($_POST['promotion']);

            $sql = "UPDATE `promotions` SET `PROMOTION_NAME`=:promotion WHERE `ID_PROMOTIONS`=:id;";

            $query = $bdd->prepare($sql);

            $query->bindValue(':promotion', $promotion, PDO::PARAM_STR);
...
<body id="main">
    ...
    <div id="uppercreaetudiant">
        <h2 id="titrepage">Modification d'une promotion</h2>
        <button type="button" class="btn btn-dark" onclick="self.location.href='Tableau_Promotions.php'"><i class="bi bi-arrow-left"></i> Retour au tableau</button>
    </div>


    <form method="post">
        <div class="container-md">
            <h3 class="titreformulaire">Descriptif de la promotion</h3>

            <div class="input-group mb-3">
                <span class="input-group-text" id="inputGroup-sizing-default">Nom de la promotion (format : nom - année)</span>
                <input type="text" class="form-control" aria-label="Sizing example input"
                    aria-describedby="inputGroup-sizing-default" id="promotion" name="promotion" value="<?= $result['PROMOTION_NAME'] ?>">
            </div>
